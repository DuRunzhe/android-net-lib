package com.pull2me.android.netlib.request;
...
import com.pull2me.android.netlib.handler.ResponseHander;
import com.pull2me.android.netlib.thread.DefaultThreadPool;
...
public class HttpRequest implements Runnable {
    ...
    private HttpRequest(Context context, String requestUrl, Map<String, String> requestParams, RequestMethod method, IRequestCallBack callBack, Map<String, String> requestHeaders, Object tag, int id) {
    ...
    public void run() {
        ...
        try {
            if (method == RequestMethod.GET) {
                response = UrlConnectionHelper.syncBufferGet(requestUrl, netConf, requestParams, requestHeaders);
            } else if (method == RequestMethod.POST) {
                response = UrlConnectionHelper.syncBufferPost(requestUrl, netConf, requestParams, requestHeaders);
            } else {
                //TODO
            }
            //设置请求结束时间
    ...
    private HttpResponse syncRun() {
        ...
        try {
            if (method == RequestMethod.GET) {
                response = UrlConnectionHelper.syncGet(requestUrl, netConf, requestParams, requestHeaders);
            } else if (method == RequestMethod.POST) {
                response = UrlConnectionHelper.syncPost(requestUrl, netConf, requestParams, requestHeaders);
            }
        } catch (Exception e) {
